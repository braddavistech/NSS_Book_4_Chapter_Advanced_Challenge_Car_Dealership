...
const bankLoan = (carSales => {
  let monthArray = [];
  for (let i = 1; i < carSales.length; i++) {
    monthArray.push(carSales[i].credit.credit_provider);
  } 
  monthArray.sort();
  let currentBank = null;
  let currentBankCount = 0;
  for (let i = 0; i < monthArray.length; i++) {
    if (monthArray[i] != currentBank) {
      if (currentBankCount > bankCount) {
        popBank= currentBank;
        bankCount = currentBankCount;
      }
      currentBank= monthArray[i];
      currentBankCount = 1;
    } else {
      currentBankCount = currentBankCount + 1;
    }
  }  
});

const popModel = (carSales => {
  let monthArray = [];
  for (let i = 1; i < carSales.length; i++) {
    monthArray.push(carSales[i].vehicle.model);
  } 
  monthArray.sort();
  let currentModel= null;
  let currentCount = 0;
  for (let i = 0; i < monthArray.length; i++) {
    if (monthArray[i] != currentModel) {
      if (currentCount > popCount) {
        popCar= currentModel;
        popCount = currentCount;
      }
      currentModel= monthArray[i];
      currentCount = 1;
    } else {
...
const grossProfit = (carSales => {
  for (let i = 0; i < carSales.length; i++) {
     yearlyTotal += carSales[i].gross_profit;
  }
  return yearlyTotal;
});

const salesLead = (carSales => {
  let monthArray = [];
  for (let i = 1; i < carSales.length; i++) {
    monthArray.push(carSales[i].sales_agent.first_name + " " + carSales[i].sales_agent.last_name);
  } 
  monthArray.sort();
  let currentSalesman = null;
  let currentSalesCount = 0;
  for (let i = 0; i < monthArray.length; i++) {
    if (monthArray[i] != currentSalesman) {
      if (currentSalesCount > salesCount) {
        salesMan = currentSalesman;
        salesCount = currentSalesCount;
      }
      currentSalesman = monthArray[i];
      currentSalesCount = 1;
    } else {
      currentSalesCount = currentSalesCount + 1;
    }
  }  
});

const indivProf = (carSales => {
  let monthArray = [];
  for (let i = 1; i < carSales.length; i++) {
    let temp = {};
    temp.name = (carSales[i].sales_agent.first_name + " " + carSales[i].sales_agent.last_name);
    temp.prof = carSales[i].gross_profit;
  ...
  let totalTracker = [];
  for (let i = 0; i < monthArray.length; i++) {
    if (findObjectByKey(totalTracker, "name", monthArray[i].name) == null) {
      totalTracker.push(monthArray[i]);
    } else {
      let tempNumber = findObjectByKey(totalTracker, "name", monthArray[i].name);
      totalTracker[tempNumber].prof += monthArray[i].prof;
    }
  }  
  for (let i = 0; i < totalTracker.length; i++) {
      if (totalTracker[i].prof > profAmt) {
        profAmt = totalTracker[i].prof;
        profName = totalTracker[i].name;
      }
  }
});


const monthSales = (carSales => {
  let monthArray = []
  for (let i = 1; i < carSales.length; i++) {
    monthArray.push(carSales[i].purchase_date.substr(5,2));
...
function printScreen (yearlyTotal, highCount, highMonth, salesMan, salesCount, profName, profAmt, popCar, popCount, popBank, bankCount) {
...
salesLead(carSales);
monthSales(carSales);
let money = grossProfit(carSales);
printScreen(yearlyTotal, highCount, highMonth, salesMan, salesCount, profName, profAmt, popCar, popCount, popBank, bankCount);